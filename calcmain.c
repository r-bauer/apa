...
{
  struct TermData *terml,       //two terms and the solution
   *term2, *solution;
  int operator;
  ...
  if (terml == NULL || term2 == NULL || solution == NULL)
    {
  ...
  if (argc < 2)                 // no input file specified, hence
    {                           // it must be interactive, 
      ...
      if (*buffer == 'q' || *buffer == 'Q')
        ...
        {
          format = AsciiToTerm (buffer, terml);
          if (!format)
            {
      ...
      printf ("Type operation (+-* /) and press <Enter>\n");
      operator = getchar ();
      ...
      if (operator != '+' &&
          operator != '-' && operator != '*' && operator != '/')
        {
          ...
          if (!ComputeResult (terml, operator, term2, solution))
            return (EXIT_FAILURE);
      ...
      else                      // calculator operations in a file
        {
          ...
          if (argc > 2)
            ...
            strcpy (fout_name, " longmath.outj");
          ...
          while (!feof (fin))
            {
              // get the first term
              format = GetFileTerm (terml, fin, fout);
              if (!format)
                break;
...
// 
int
GetFileOperator (int *operator, FILE * infile, FILE * outfile)
{
  ...
  if (p == NULL)
    {
      ...
      fprintf (outfile, "Error: invalid operator :\n%s\n", buffer);
      return (0);
    }
  else
    *p = '\0';
  *operator = * buffer;
  if (*(buffer + 1) != '\0' ||
       ...
       *operator != '-' &&
       *operator != '*' &&
       *operator != '/' && *operator != 'c' && *operator != 'C'))
    {
...
int
GetFileTerm (struct TermData *t, FILE * infile, FILE * outfile)
{
  char buffer[MAX_SIZE + 4];
  char *p;
  int format = NORMAL;
  ...
  if (p == NULL)
    {
      ...
      fprintf (outfile, "Error: term too long:\n%s\n", buffer);
      return (0);
    }
  else
    *p = '\0';
  ...
  if (t->sign == 0 || (t->places_before + t->places_after > MAX_SIZE))
    {
      printf ("Error: term too long or invalid:\n%s\n", buffer);
      fprintf (outfile, "Error: term too long or invalid:\n%s\n", buffer);
      return (0);
    }
  fprintf (outfile, "%s\n", buffer);
  return (format);
